...
export const inventarioController = {
   ...
   */
  getInventario: asyncHandler(async (req: Request, res: Response) => {
   ...
   */
  updateProducto: asyncHandler(async (req: Request, res: Response) => {
    const { negocioId } = req.auth;
    const { id } = req.params;
   ...
   */
  deleteProducto: asyncHandler(async (req: Request, res: Response) => {
    const { negocioId } = req.auth;
    const { id } = req.params;
  ...
  addStock: asyncHandler(async (req: Request, res: Response) => {
    ...
    const isInvalidId = (id: any) => !id || id === "null" || id === "undefined" || id === "";
    ...
    const result = await inventarioService.addStock(
      ...
      cleanCuentaId, monto, moneda, cleanProveedorId, estadoPago
    );
   ...
   */
  getProductoHistory: asyncHandler(async (req: Request, res: Response) => {
    const { negocioId } = req.auth;
    const { id } = req.params;
    const history = await inventarioService.getProductoHistory(negocioId, id as string);
   ...
   */
  getProductoSales: asyncHandler(async (req: Request, res: Response) => {
    const { negocioId } = req.auth;
    const { id } = req.params;
    const { page, limit } = req.query;
    
    const result = await inventarioService.getProductoSales(
      negocioId, 
      id as string, 
      page ? parseInt(page as string) : undefined,
      limit ? parseInt(limit as string) : undefined
   ...
   */
  getProductoMovimientos: asyncHandler(async (req: Request, res: Response) => {
    const { negocioId } = req.auth;
    const { id } = req.params;
    const { page, limit } = req.query;
    
    const result = await inventarioService.getProductoMovimientos(
      negocioId, 
      id as string, 
      page ? parseInt(page as string) : undefined,
      limit ? parseInt(limit as string) : undefined
