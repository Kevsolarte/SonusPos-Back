...
export const negocioRepository = {
  ...
  async getLimits(negocioId: string) {
    return await prisma.negocio.findUnique({
      where: { id: negocioId },
      select: { 
        limiteUsuarios: true, 
  ...
  async updateNegocio(negocioId: string, data: any) {
  ...
  async getSubscriptionData(negocioId: string) {
  ...
  async createSuscripcionPago(negocioId: string, data: any) {
