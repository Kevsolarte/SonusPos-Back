...
export const ventasService = {
    async createVenta(negocioId: string, userId: string, dto: createVentaFullType) {
    ...
    async calcularTotalesVenta(negocioId: string, detalles: { productoId: string; cantidad: string; varianteId?: string | null }[], descuentoPorcentaje: number = 0) {
        ...
        for (const detalle of detalles) {
            ...
            if (detalle.varianteId && (producto as any).variantes) {
                const variante = (producto as any).variantes.find((v: any) => v.id === detalle.varianteId);
                if (variante) {
            ...
            const promoRel = (producto as any).promociones?.[0];
            if (promoRel) {
            ...
            detallesCalculados.push({
                ...
                esCombo: (producto as any).esCombo,
                componentes: (producto as any).componentes || [],
    ...
    async calcularTotales(negocioId: string, dto: calcularTotalesType) {
        ...
        return await this.calcularTotalesVenta(
            negocioId, 
            body.detalles.map(d => ({ ...d, varianteId: d.varianteId || null }))
        );
    },
