...
export const ProveedorController = {
  ...
  getById: asyncHandler(async (req: Request, res: Response) => {
    const { id } = req.params;
    const { negocioId } = req.auth;
    const proveedor = await ProveedorService.getById(id as string, negocioId);
  ...
  update: asyncHandler(async (req: Request, res: Response) => {
    const { id } = req.params;
    const { negocioId } = req.auth;
    const actualizado = await ProveedorService.update(id as string, negocioId, req.body);
  ...
  delete: asyncHandler(async (req: Request, res: Response) => {
    const { id } = req.params;
    const { negocioId } = req.auth;
    await ProveedorService.delete(id as string, negocioId);
