...
export const saasGuard = {
   ...
   */
  async canCreateProduct(negocioId: string) {
    ...
    if (negocio._count.productos >= negocio.limiteProductos) {
      throw new AppError(
        `Límite de productos alcanzado (${negocio.limiteProductos}). Sube de nivel tu plan para agregar más.`, 
        403
      );
    }
  },
   ...
   */
  async canCreateUser(negocioId: string) {
    const negocio = await prisma.negocio.findUnique({
      where: { id: negocioId },
      select: { 
        limiteUsuarios: true, 
    ...
    if (negocio._count.users >= negocio.limiteUsuarios) {
      throw new AppError(
        `Límite de usuarios alcanzado (${negocio.limiteUsuarios}). Sube de nivel tu plan para agregar más.`, 
        403
      );
    }
  }
