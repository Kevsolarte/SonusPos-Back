...
export const dashboardRepository = {
    ...
    async getSaldosCuentas(negocioId) {
    ...
    async getResumenCreditos(negocioId) {
    ...
    async getTopDeudas(negocioId) {
        // Deudas pendientes
        const deudas = await prisma.deuda.findMany({
            where: {
                negocioId,
                monto: { gt: prisma.deuda.fields.montoPagado },
            },
            include: {
    ...
    async getMetodosPagoDistribucion(negocioId) {
        const today = new Date();
        today.setHours(0, 0, 0, 0);
    ...
    async getCajasActivas(negocioId) {
        ...
        return cierres.map(c => {
            const perms = c.user.permissions;
            return {
                id: c.id,
                usuario: c.user.name,
