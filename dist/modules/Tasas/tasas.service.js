 ...
 */
async function recalcularSaldosVES(negocioId, nuevaTasa) {
    const cuentas = await prisma.cuenta.findMany({
        where: { negocioId, activo: true },
    });
    if (cuentas.length === 0)
        return;
    await prisma.$transaction(cuentas.map((c) => prisma.cuenta.update({
        where: { id: c.id },
...
export const tasaService = {
    async getAll(negocioId) {
        ...
        try {
            ...
            const bcv = dataDolar.find((r) => r.fuente === 'oficial');
            const par = dataDolar.find((r) => r.fuente === 'paralelo');
            // 2. Obtener Euros (Oficial)
            const resEuro = await fetch('https://ve.dolarapi.com/v1/euros');
            const dataEuro = await resEuro.json();
            const eur = dataEuro.find((r) => r.fuente === 'oficial');
    ...
    async update(negocioId, id, dto) {
        ...
        let monedaAfectada = null;
        if (data.isPrincipal) {
            const existing = await tasaRepository.findById(id, negocioId);
            if (!existing)
                throw new AppError("Tasa no encontrada", 404);
