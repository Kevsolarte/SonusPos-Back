...
function derivarEstado(monto, montoPagado, fechaVencimiento) {
...
function enriquecer(item) {
    return {
        ...item,
        estado: derivarEstado(Number(item.monto), Number(item.montoPagado), item.fechaVencimiento ?? null),
        saldoPendiente: Number(item.monto) - Number(item.montoPagado),
    };
}
// ─── Helper: tasa activa ─────────────────────────────────────────────────────
async function getTasaVES(negocioId) {
    const t = await prisma.tasaCambio.findFirst({
...
export const cobrosRepository = {
    async findAll(negocioId, params) {
    ...
    async delete(id, negocioId) {
        const cobro = await prisma.cobro.findFirst({ where: { id, negocioId } });
        if (!cobro)
        ...
        return await prisma.cobro.delete({ where: { id } });
    },
    /**
     ...
     */
    async registrarAbono(id, negocioId, data) {
        const cobro = await prisma.cobro.findFirst({ where: { id, negocioId } });
        if (!cobro)
            throw new AppError("Cobro no encontrado", 404);
...
export const deudasRepository = {
    async findAll(negocioId, params) {
        ...
        const [items, total] = await Promise.all([
            ...
            prisma.deuda.count({ where: { negocioId } }),
        ]);
        const enriquecidos = items.map(enriquecer);
        const filtrados = params?.estado
            ? enriquecidos.filter((d) => d.estado === params.estado)
            : enriquecidos;
    ...
    async delete(id, negocioId) {
        const deuda = await prisma.deuda.findFirst({ where: { id, negocioId } });
        if (!deuda)
        ...
        return await prisma.deuda.delete({ where: { id } });
    },
    /**
...
//# sourceMappingURL=creditos.repository.js.map